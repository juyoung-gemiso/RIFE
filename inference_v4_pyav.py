...
class RIFE:
    ...
    def build_read_buffer(self, read_buffer, container):
        try:
            for frame in container.decode(video=0):
                read_buffer.put(self.frame2image(frame))
        except:
            pass
    ...
    def run(self, video:str, extension:str, fps:float|int, bitrate, scale:float=1.0):
        ...
        lastframe = self.frame2image(next(container.decode(video=0)))
        ...
        _thread.start_new_thread(self.build_read_buffer, (read_buffer, container))
