...
class RIFE:
    ...
    def run(self, video:str, extension:str, fps:float|int, bitrate, scale:float=1.0):
        ...
        while True:
            ...
            selected_frames = [one_second_frames[0]]
            ...
            while one_second_frame_index < len(one_second_frames) - 1:
                ...
                while temp_similarity >= 0.99:
                    ...
                    if one_second_frame_index == len(one_second_frames) - 1: break
            ...
            if len(selected_frames) - 2 == 0:
                added_frame_count = round(fps) - 1
            else:
                added_frame_count = math.ceil((round(fps) - len(selected_frames) - 1) / (len(selected_frames) - 2))
            if args.debug: print(f"added_frame_count: {added_frame_count}")
            added_frame_negative_count = len(similarities) * (1 + added_frame_count) - round(fps) # added_frame_count - 1 만큼 보간 처리해야하는 프레임 개수
            insert_count_per_indexs = [added_frame_count] * (len(similarities) - added_frame_negative_count) + [added_frame_count - 1] * added_frame_negative_count
            insert_count_per_indexs = list(map(lambda x: x[1], sorted(list(enumerate(insert_count_per_indexs)), key=lambda x: similarities[x[0]])))
            input_image_index = 1
            I0 = input_images[0]
            while input_image_index < len(input_images):
                ...
                if add_count > 0:
                    ...
                    output = []
                current_interpolated_frames = []
                current_interpolated_frames.append((selected_frames[input_image_index - 1], -1))
                for mid in output:
            ...
            one_second_frames = [selected_frames[-1]]
...
if __name__ == '__main__':
    ...
    Example Command Line:
        ...
        > python inference_v2.py --video=C:\Users\gemiso\Desktop\frame_interpol_videos\\son_goal_01_25fps.mp4 --fps=29.97 --bitrate=60M
        > python inference_v2.py --video=C:\Users\gemiso\Desktop\frame_interpol_videos\\son_goal_01_25fps.mp4 --fps=59.94 --bitrate=60M
    """
