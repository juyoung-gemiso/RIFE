...
class RIFE:
    def __init__(self, model_dir, debug=False, save_img=False):
        self.debug = debug
        self.img_dir = r"Z:\AI_workspace\video_interpolation_backup\uniform_distributed_imgs"
    ...
    def run(self, img_dir:str, extension:str, fps:float|int, bitrate, scale:float=1.0):
        global IDX, IS_TOP
        if self.debug:
            start_time = time.time()
        videoCapture = os.listdir(img_dir)
        original_fps = 50
        ...
        _thread.start_new_thread(self.build_read_buffer, (read_buffer, videoCapture, img_dir))
            ...
            for inserted_index in range(interval, round(original_fps) + 1, interval):
                ...
                for original_frame_index in range(original_frame_start_index, inserted_index):
                    if self.save_img:
                        ...
                        pil_img.save(f"{self.img_dir}/{self.idx}_{'top' if IS_TOP else 'bottom'}.tga")
        ...
        # try:
        #     transferAudio(img_dir, vid_out_name, fps, bitrate)
        # except:
...
if __name__ == '__main__':
    ...
    Example Command Line:
        ...
        > python inference_v5.py --img-dir=Z:\\AI_workspace\\video_interpolation_backup\\soccer_25fps_1m30s_frames --fps=59.94 --bitrate=60M
    """
    ...
    rife.run(args.img_dir, args.ext, args.fps, args.bitrate, scale=0.5)
